...
{
    private readonly HashSet<SyntaxNode> _ignoreOperations = new();
    ...
    public override void VisitBinaryExpression(BinaryExpressionSyntax node)
    {
        ...
        if (nodeKind is not SyntaxKind.LogicalAndExpression and not SyntaxKind.LogicalOrExpression)
        {
    ...
    private bool IsRecursiveCall(InvocationExpressionSyntax syntax)
    {
        ...
        {
            LocalFunctionStatementSyntax localFun when IsSameNameAndParameterCount(localFun.Identifier, localFun.ParameterList) => true,
            MethodDeclarationSyntax method when IsSameNameAndParameterCount(method.Identifier, method.ParameterList) => true,
            ClassDeclarationSyntax _ or StructDeclarationSyntax _ or NamespaceDeclarationSyntax _ or CompilationUnitSyntax _ => false,
            _ => Inner(node?.Parent),
        };
